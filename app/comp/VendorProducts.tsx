...
const VendorProducts: React.FC = () => {
  ...
  const API_URL = "https://backendforworld.onrender.com/api/vendorproducts";
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        await AsyncStorage.setItem("cachedVendorProducts", JSON.stringify(allProducts));
      } catch (error) {
        console.error("❌ Error fetching vendor products:", error);
        const cachedData = await AsyncStorage.getItem("cachedVendorProducts");
        if (cachedData) {
  ...
  useEffect(() => {
    AsyncStorage.setItem("vendorProductsQuantities", JSON.stringify(cartQuantities));
  }, [cartQuantities]);
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.card}>
            <TouchableOpacity
              onPress={async () => {
                await AsyncStorage.setItem("selectedVendorProduct", JSON.stringify(item));
                router.push("/comp/VendorProductDetails");
              }}
            >
              <Image
                source={{
